"""双流骨架拼接 CTC 训练：hand 42 + pose 33 = 75 节点单 STGCN。

动机：骨架 baseline（0.740）只用 hand 42 节点；pose 33 点（全身姿态，
含手臂运动）从未单独参与骨架实验。拼接 75 节点图（分块对角：
hand 子图 + pose 子图，跨子图无边），验证全身信息对骨架模态的增益。

数据：data/dataset/{split}.npz（hand，T,42,3）+ {split}_pose.npz（T,33,3）
      ——已验证同索引同帧数完全对齐；pose NaN → 0。
模型：STGCNCTC（9 层），adjacency = build_hand_pose_graph(hand42, pose33)。
保存：checkpoints/best_dual.pt（不覆盖 best.pt）

用法: python scripts/train/train_full_dual.py --augment
"""
...
class DualSkeletonDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        x = np.transpose(data, (2, 0, 1))       # (3, T, 75)
...
def load_split_dual(base: Path, split: str, vocab_idx: dict | None,
                    min_det: float, target_t: int, max_samples: int = 0,
                    verbose: bool = True):
    """同时加载 hand + pose，逐段拼接成 (T, 75, 3)。
    ...
    if verbose:
        print(f"[数据] {split}.npz + {split}_pose.npz 解压"
    ...
    samples = []
    for i in keep:
        h = np.asarray(data_arr[i], dtype=np.float32)
        p = np.asarray(pose_img[i], dtype=np.float32)
        assert len(h) == len(p), f"{split} 段 {i} hand/pose 帧数不一致"
        samples.append(np.concatenate([h, np.nan_to_num(p)], axis=1))
    glosses = [str(gloss_arr[i]) for i in keep]
    if verbose:
        print(f"[数据]   拼接 {len(samples)} 段 → (T,75,3)"
              f"（{time.monotonic() - t0:.0f}s）", flush=True)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="双流骨架拼接 CTC 训练")
    ...
    parser.add_argument("--checkpoint", type=str,
                        default="checkpoints/best_dual.pt")
    ...
    for p in (train_path, dev_path, vocab_path,
              data_dir / "train_pose.npz", data_dir / "dev_pose.npz"):
        if not p.exists():
            print(f"缺少 {p}")
            return 1

    # 75 节点图：hand 42 分块对角 + pose 33，跨子图无边
    adjacency = build_hand_pose_graph(
        build_hand_graph(num_hands=2),
        build_adjacency(POSE_CONNECTIONS, 33))
    print(f"双流图: hand 42 + pose 33 = {adjacency.shape[0]} 节点")
    ...
    if args.eval_only:
        ...
        model = STGCNCTC(num_classes=len(vocab), adjacency=adjacency).to(device)
        model.load_state_dict(ckpt["state_dict"])
        for split in args.eval_splits:
            if not (data_dir / f"{split}.npz").exists():
                continue
            samples, y, ylen, glosses = load_split_dual(
                data_dir, split, vocab_idx, 0.0, args.target_t)
            x = to_tensor_batch(
    ...
    train_samples, y_train, ylen_train, _ = load_split_dual(
        data_dir, "train", vocab_idx, MIN_DETECTION, args.target_t,
        args.max_samples)
    dev_samples, y_dev, ylen_dev, _ = load_split_dual(
        data_dir, "dev", vocab_idx, MIN_DETECTION, args.target_t,
        args.max_samples)
    ...
    train_ds = DualSkeletonDataset(train_samples, y_train, ylen_train,
